...
export default function ChatPage() {
  ...
  const exportToMarkdown = () => {
    if (!structure) {
      alert('構造化データがありません。メッセージを送信してから再度お試しください。');
      return;
    }

    const markdown = generateMarkdown(structure, messages, conversationTitle);
    downloadMarkdown(markdown, conversationTitle || 'チャット履歴');
  };
  ...
  const saveConversation = async () => {
    ...
    setIsSaving(true);
    try {
      // ローカルストレージに保存（既存の/importページと同じ形式）
      const conversationData = {
        id: Date.now().toString(),
        title: conversationTitle || '内蔵チャット履歴',
      ...
      const existingData = JSON.parse(localStorage.getItem('chat2doc_conversations') || '[]');
      existingData.unshift(conversationData);
      
      // 最新20件のみ保持
      localStorage.setItem('chat2doc_conversations', JSON.stringify(existingData.slice(0, 20)));
      
      alert('会話が正常に保存されました！ダッシュボードで確認できます。');
    } catch (error) {
...
// Markdown生成関数
function generateMarkdown(structure: ChatStructure, messages: ChatMessage[], title: string): string {
...
${structure.progress.length > 0 ? `### ✅ 対応履歴
${structure.progress.map(item => `- ${item}`).join('\n')}` : ''}

${structure.challenges.length > 0 ? `### ⚠️ 課題
${structure.challenges.map(item => `- ${item}`).join('\n')}` : ''}

${structure.nextActions.length > 0 ? `### 📋 次のアクション
${structure.nextActions.map(action => {
  let actionText = `- ${action.title}`;
  if (action.owner) actionText += ` (担当: ${action.owner})`;
  if (action.due) actionText += ` (期限: ${action.due})`;
  return actionText;
}).join('\n')}` : ''}

${structure.code.length > 0 ? `### 💻 コード
${structure.code.map((codeItem, index) => `
#### ${codeItem.fileName || `コードスニペット ${index + 1}`}
${codeItem.description ? `**説明**: ${codeItem.description}\n` : ''}
\`\`\`
${codeItem.snippet}
\`\`\`
`).join('\n')}` : ''}

${structure.intentions.length > 0 ? `### 💡 意図
${structure.intentions.map(item => `- ${item}`).join('\n')}` : ''}

${structure.concerns.length > 0 ? `### 🚨 懸念点
${structure.concerns.map(item => `- ${item}`).join('\n')}` : ''}

${structure.tags.length > 0 ? `### 🏷️ タグ
${structure.tags.map(tag => `\`${tag}\``).join(' ')}` : ''}

${structure.links.length > 0 ? `### 🔗 関連リンク
${structure.links.map(link => `- [${link.label}](${link.url})`).join('\n')}` : ''}
...
*Generated by [Chat2Doc](https://chat2doc.panolabollc.com) - AI会話を知識資産に変換*
`;
}
...
function downloadMarkdown(content: string, filename: string) {
  ...
  link.href = url;
  link.download = `${filename.replace(/[^a-zA-Z0-9\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/g, '_')}.md`;
