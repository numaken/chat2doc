...
### 要件
- LP内の「登録不要／すぐ使える」断定表現を撤去し、下記に統一：
  - **未ログイン**：/import でお試し可（ローカル保存のみ）
  - **ログイン**：同期・共有・履歴保存（β）
- ヒーロー直下に 1 行注記＋CTA 2本（ログイン／ログイン不要で試す）

### 受入基準
- LPから /import へ 1 クリック導線がある
- 文言矛盾がない（断定の「登録不要」表現はLPに残っていない）

## 17. モデル表記の抽象化

### 要件
- LPの固定モデル名を削除し、「高速（軽量）／高精度（リッチ）」に置換
- 具体モデルは README の「モデル切替」に集約（環境変数説明付き）
